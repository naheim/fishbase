<?php
	...
	function getPhyloDiv($pagestring, $taxonId) {
		$myReturn = array('pdiv' => NULL, 'pdivref' => NULL, 'tlevel' => NULL, 'tlevelse' => NULL, 'tlevelref' => NULL, 
			'resil' => NULL, 'resilref' => NULL, 'resil_K' => NULL, 'resil_tm' => NULL, 'resil_tmax' => NULL, 'resil_fec' => NULL,
			'vulnerab' => NULL, 'vulnerabref' => NULL);
		...
		if(count($x)==4) {
			...
			if(count($ref) == 4) $myReturn['vulnerabref'] = trim($ref[2]);
			unset($ref);
		} 
		...
		if(count($x)==4) {
			...
			$ref = array();
			$y = preg_match("#(".$beginstr.")(.*?)(".$endstr.")#i", $selectedstr, $ref);			
			if(count($ref) == 4) $myReturn['resilref'] = trim($ref[2]);
			unset($ref);
	...
	for($k=0; $k<count($randsample); ++$k) {				
		...
		if(is_countable($feed) && count($feed) > 0 && count($feed->taxon) > 0) {		
			...
			$file_row = $taxon_info."\t".$size_info."\t".$habitat_info1."\t".$habitat_info2."\t".$habitat_info3."\t".$other_info."\n";
